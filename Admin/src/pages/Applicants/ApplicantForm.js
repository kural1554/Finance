...
import PDFPreview from "../Forms/FormWizard/PDFdocument"; // Assuming this exists
import { initialFormData } from "../Forms/constants/formConfig"; // Assuming this exists
import validateForm from "../Forms/utils/validation"; // Assuming this exists
import DocumentUpload from "./IdProofSection"; // Import the separate component
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { useLocation } from "react-router-dom";
import formConfig from '../Forms/constants/formConfig';
...
const ApplicantForm = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  // --- Create FormData ---
  const submissionData = new FormData(); // Define submissionData before try block
  ...
  safeAppend('userID', formData.userID); // userID yen anupreenga nu check pannunga, backend la set panradhu better
  safeAppend('title', formData.title);
  safeAppend('first_name', formData.first_name);
  safeAppend('last_name', formData.last_name);
  safeAppend('dateOfBirth', formData.dateOfBirth);
  ...
  if (formData.employmentType || formData.jobTitle) { 
    ...
    safeAppend('employment[0][otherIncome]', formData.otherIncome);
  }
  ...
  if (formData.propertyType || formData.property_address) {
  ...
  // --- OREY TRY-CATCH BLOCK KULLA TOKEN AND AXIOS CALL ---
  try {
    const token = localStorage.getItem("accessToken"); // Unga token key correct-aa irukkanum

    if (!token) {
      toast.error("Authentication token not found. Please login again.");
      console.error("Token not found for API request");
      return; // Exit if no token
    }

    // --- Send FormData using Axios WITH Authorization header ---
    const response = await axios.post(
      `${process.env.REACT_APP_API_BASE_URL}api/applicants/applicants/`,
      submissionData, // Pass the FormData object
      {
        headers: {
          // 'Content-Type': 'multipart/form-data' will be set by browser for FormData
          'Authorization': `Bearer ${token}` // <--- MIKAVUM MUKKIYAM!
        }
      }
    );
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
          errorMsg = "Unauthorized. Your session might have expired or token is invalid. Please login again.";
      } else if (typeof error.response.data === 'object' && error.response.data !== null) {
        const errors = error.response.data;
        if (errors.detail) {
          errorMsg = errors.detail;
        } else if (errors.errors && typeof errors.errors === 'object') { // Backend error format like {'errors': {'field': ['message']}}
            errorMsg = Object.entries(errors.errors)
              .map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join(', ') : value}`)
              .join('; ');
        } else if (Array.isArray(errors.non_field_errors)) {
          errorMsg = errors.non_field_errors.join(' ');
        } else {
          // Generic fallback for other object errors from backend
      ...
      errorMsg = "Submission failed: No response from server. Please check network.";
    } else {
